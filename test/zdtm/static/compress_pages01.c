...
/*
 * Allocate shared anonymous memory, fill it with a known pattern,
 * fork a child, then have the child modify some pages (triggering
 * CoW). After C/R, verify that:
 *
 ...
 *
 * This exercises compression of both shared and private (CoW) pages
 * in a multi-process tree.
 */
...
static int child_fn(char *region, int pipe_fd)
{
	...
	for (i = COW_PAGES * (int)PAGE_SIZE; i < NR_PAGES * (int)PAGE_SIZE; i++) {
		if ((unsigned char)region[i] != PATTERN_PARENT) {
			fail("child: shared page byte %d is 0x%02x, expected 0x%02x",
