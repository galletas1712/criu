...
static int restore_memfd_shmem_content_ex(int fd, unsigned long shmid, unsigned long size)
{
...
{
	return do_restore_shmem_content_ex(addr, si->size, si->shmid);
}

int restore_sysv_shmem_content(void *addr, unsigned long size, unsigned long shmid)
{
	return do_restore_shmem_content_ex(addr, round_up(size, PAGE_SIZE), shmid);
}

int restore_memfd_shmem_content(int fd, unsigned long shmid, unsigned long size)
{
	return restore_memfd_shmem_content_ex(fd, shmid, size);
}
...
static int open_shmem(int pid, struct vma_area *vma)
{
	...
	if (use_memfd) {
		if (restore_memfd_shmem_content_ex(f, vi->shmid, si->size) < 0) {
		...
		if (do_restore_shmem_content_ex(addr, si->size, vi->shmid) < 0) {
